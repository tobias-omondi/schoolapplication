...
const Counter = ({ target, duration, label }) => {
  ...
  return (
    ...
    >
      <h1 className="text-2xl font-semibold text-blue-600 underline underline-offset-4 decoration-red-950">
        {display}%
      </h1>
      <p className="text-gray-700 text-xl font-light">{label}</p>
    </motion.div>
  )
}
...
const AcademicResults = () => {
  return (
    <div>
      ...
      transition = {{duration:0.8}}
      viewport={{once:true}}
       className="grid grid-cols-2 md:grid-cols-4 gap-6 mt-10 px-5 text-center ">
      ...
      {/* Intro */}
      <motion.div className='mt-5 px-3'
      initial = {{opacity:0, y:90}}
      whileInView = {{opacity:1, y:0}}
      transition = {{duration:0.8}}
      viewport={{once:true}}>
      ...
      <motion.div
       ...
       viewport={{once:true}}
       className='mt-10 text-center'>
        <h1 className='text-gray-600 text-sm'>Congratulation Class of 2025!</h1>
      </motion.div>
