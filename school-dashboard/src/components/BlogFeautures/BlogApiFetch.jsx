...
const BlogApiFetch = () => {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        ...
        setSavedBlogs(response.data.slice(0, 3)); // only recent 3
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Reset form
      setBlogData({ title: '', content: '', category: 'General', imageUrl: '', tags: '', isPublished: false });
      setEditingId(null);

      // Update state instantly
      if (editingId) {
        setSavedBlogs(prev => prev.map(b => b.id === editingId ? updatedBlog : b));
      } else {
  ...
  const handleEdit = (item) => {
    ...
    Swal.fire('Edit Mode', 'You are now editing this blog post.', 'info');
  };
  ...
  const handleSaveDraft = () => {
    ...
    Swal.fire('Draft Saved', 'This blog is saved as draft (not published).', 'info');
  };
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 py-8 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-gray-600 text-center mb-8">
          {editingId ? 'Edit your blog content below' : 'Publish school blogs easily using our editor'}
        </motion.p>
            ...
            <div className="bg-white rounded shadow-lg overflow-hidden">
              ...
              <form onSubmit={handleSubmit} className="p-6">
                ...
                <div className="mb-6">
                  ...
                  <motion.input whileFocus={{ scale: 1.01 }} type="text" id="title" name="title" value={blogData.title} onChange={handleInputChange}
                    placeholder="Headline for your blog..." className="w-full px-4 py-3 border border-gray-300 rounded text-gray-700 focus:ring-2 focus:ring-blue-500 focus:outline-none focus:border-transparent transition" required />
                </div>

                <div className="mb-6">
                  <label htmlFor="content" className="block text-sm font-medium text-gray-700 mb-1">Content</label>
                  <motion.textarea whileFocus={{ scale: 1.01 }} id="content" name="content" value={blogData.content} onChange={handleInputChange}
                    rows="10" placeholder="Write the full blog content here..." className="w-full px-4 py-3 border border-gray-300 rounded focus:ring-2 text-gray-700 focus:outline-none focus:ring-blue-500 focus:border-transparent transition" required />
                </div>
                ...
                <div className="mb-8">
                  ...
                  <div className="flex items-center">
                    ...
                    <input type="text" id="imageUrl" name="imageUrl" value={blogData.imageUrl} onChange={handleInputChange}
          ...
          <motion.div initial={{ x: 50, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.5 }} className="w-full lg:w-1/3">
            <div className="bg-white rounded-xl shadow-lg p-6 sticky top-6">
              <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center">
                <IoIosCreate className="mr-2 text-blue-500" /> Recent Blogs
              </h3>
              {isLoading && savedBlogs.length === 0 ? (
                ...
                <p className="text-gray-500 text-center py-4">No blogs yet. Create your first post!</p>
              )}

              <div className="mt-8">
                <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center">
                  <IoIosImages className="mr-2 text-blue-500" /> Tips for School Blogs
                </h3>
